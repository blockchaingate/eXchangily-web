...
@Component({
  ...
  standalone: true,
  imports: [PinNumberModal, MatFormFieldModule, MatSelectModule, MatInputModule, FormsModule, TranslateModule],
  ...
  getStakeInfo() {
    ...
    .subscribe((res: any) => {
        ...
        if(res && res.executionResult) {
          ...
          if(output) {
            const hashes: any = this.web3Serv.decodeParameters(['bytes32[]'], output)[0];
