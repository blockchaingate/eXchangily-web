...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, MatIconModule, MatRadioModule, MatSlideToggleModule, FormsModule, BindpayComponent, PinNumberModal, TranslateModule],
...
export class MyordersComponent implements OnInit, OnDestroy {
    ...
    async ngOnInit() {
        ...
        if (this.wallet) {
            this.storageServ.getIsMobile().subscribe(isMobile => {
                if (!isMobile) {
    ...
    async openWithdrawModal(template: TemplateRef<any>) {
        ...
        if (this.coinName == 'USDT') {
            ...
            const withdrawConfig = environment.minimumWithdraw[this.coinName as keyof typeof environment.minimumWithdraw];
