...
import { Signature, Token } from '../../../../models/kanban.interface';
...
import { SendCoinForm } from '../../../../models/kanban.interface';
...
import { Pair } from '../../../market/models/pair';
...
import { FilterByChainPipe } from '../../../shared/pipes/filter-by-chain.pipe';
...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, MatMenuModule, MatIconModule, TranslateModule, MatFormFieldModule, MatSelectModule,
        ReceiveCoinModal, BuyCoinModal, ManageWalletComponent, MatCheckboxModule, FormsModule, TransactionHistoryComponent,
        SortByFieldPipe, FilterByChainPipe, MatTooltipModule, FaqComponent, DepositAmountModal, RedepositAmountModal,
...
export class WalletDashboardComponent implements OnInit {
    ...
    @ViewChild('depositModal', { static: true }) depositModal: DepositAmountModal = {} as DepositAmountModal;
    @ViewChild('redepositModal', { static: true }) redepositModal: RedepositAmountModal = {} as RedepositAmountModal;
    @ViewChild('addGasModal', { static: true }) addGasModal: AddGasModal = {} as AddGasModal;
    @ViewChild('addAssetsModal', { static: true }) addAssetsModal: AddAssetsModal = {} as AddAssetsModal;
    @ViewChild('sendCoinModal', { static: true }) sendCoinModal: SendCoinModal = {} as SendCoinModal;
    @ViewChild('showSeedPhraseModal', { static: true }) showSeedPhraseModal: ShowSeedPhraseModal = {} as ShowSeedPhraseModal;
    @ViewChild('verifySeedPhraseModal', { static: true }) verifySeedPhraseModal: VerifySeedPhraseModal = {} as VerifySeedPhraseModal;
    @ViewChild('backupPrivateKeyModal', { static: true }) backupPrivateKeyModal: BackupPrivateKeyModal = {} as BackupPrivateKeyModal;
    @ViewChild('deleteWalletModal', { static: true }) deleteWalletModal: DeleteWalletModal = {} as DeleteWalletModal;
    @ViewChild('loginSettingModal', { static: true }) loginSettingModal: LoginSettingModal = {} as LoginSettingModal;
    @ViewChild('displaySettingModal', { static: true }) displaySettingModal: DisplaySettingModal = {} as DisplaySettingModal;
    @ViewChild('toolsModal', { static: true }) toolsModal: ToolsModal = {} as ToolsModal;
    @ViewChild('getFreeFabModal', { static: true }) getFreeFabModal: GetFreeFabModal = {} as GetFreeFabModal;
    @ViewChild('lockedInfoModal', { static: true }) lockedInfoModal: LockedInfoModal = {} as LockedInfoModal;
    @ViewChild('walletUpdateModal', { static: true }) walletUpdateModal: WalletUpdateModal = {} as WalletUpdateModal;

    sendCoinForm: SendCoinForm = {} as SendCoinForm;
    ...
    depositGas = false;
    bnbBalance = 0;
    coinsPrice: CoinsPrice = {} as CoinsPrice;
    pin = '';
    baseCoinBalance = 0;
    ...
    constructor(
        ...
        this.kanbanV2Serv.getKanbanStatus().subscribe(
            (res: any) => {
                if (res && res.success) {
                    const data = res.body;
                    if (data != 'live') {
    ...
    getCoinLogo(coin: MyCoin): string {
        if (coin.logo) {
            return coin.logo;
        }
    ...
    async loadWallets() {
        const isMobile = this.storageService.getIsMobile();
    ...
    refreshGas() {
        this.kanbanV2Serv.getKanbanBalance(this.wallet.excoin.receiveAdds[0].address).subscribe(
            {
                next: (resp: any) => {
    ...
    loadNewCoinsDo() {
        ...
        for (let i = 0; i < erc20Tokens.length; i++) {
            ...
            const token1SmartcontractAdd = environment.addresses.smartContract[tokenName1 as keyof typeof environment.addresses.smartContract].toString();
            const token1 = this.coinServ.initToken('ETH', tokenName1, 18, token1SmartcontractAdd, ethCoin);
        ...
        let tokenName = 'POWR';
        let tokenSmartcontractAdd = environment.addresses.smartContract[tokenName as keyof typeof environment.addresses.smartContract].toString();
        let token = this.coinServ.initToken('ETH', tokenName, 6, tokenSmartcontractAdd, ethCoin);
        this.coinServ.fillUpAddress(token, seed, 1, 0);
        myCoins.push(token);

        tokenName = 'CEL';
        tokenSmartcontractAdd = environment.addresses.smartContract[tokenName as keyof typeof environment.addresses.smartContract].toString();
        token = this.coinServ.initToken('ETH', tokenName, 4, tokenSmartcontractAdd, ethCoin);
        this.coinServ.fillUpAddress(token, seed, 1, 0);
        myCoins.push(token);
    ...
    async fabInBtc() {
        ...
        const { txHex, txHash, errMsg } = await this.coinService.sendTransaction(coin, seed, this.toAddress, 0, options, true);
        if (errMsg) {
    ...
    async sendCoinDo() {
        ...
        if (txHash) {
            ...
            const item = {
                ...
                amount: amount,
                txid: txHash,
                from: this.wallet.mycoins[this.sendCoinForm.coinIndex].receiveAdds[0].address, // Add the missing 'from' property
