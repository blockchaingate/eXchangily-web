...
export class DepositAmountModal {
    ...
    @Input() coin: MyCoin = {} as MyCoin;
    @Input() baseCoinBalance = 0;
    @Input() alertMsg = '';
    ...
    confirmations = 0;
    lan = 'en';
    depositAmountForm = {} as any;

    constructor(private tranServ: TranslateService, private apiService: ApiService,
    ...
    async sendAllAmount(event: any) {
        ...
        if (coinName === 'BTC') {
            ...
            this.depositAmountForm.patchValue({ 'depositAmount': transOut.toString() });
            this.onTextChange();
        } else if (tokenType === 'FAB' || tokenType === 'ETH') {
        ...
        if (this.firstTime && this.coin) {
            if ((this.coin.name === 'ETH') || (this.coin.tokenType === 'ETH')) {
                ...
                if (!gasPrice) {
                    ...
                    if ('gasPrice' in chain) {
                        gasPrice = chain.gasPrice;
                    } else {
                        throw new Error(`gasPrice is not defined for chain: ${chainName}`);
                    }
                }
                const chain = environment.chains[chainName as keyof typeof environment.chains];
                const envGasPrice = 'gasPriceMax' in chain ? chain.gasPriceMax : 99999999999;
                if (gasPrice > envGasPrice) {
                    gasPrice = envGasPrice;
                }
                this.depositAmountForm.patchValue({ 'gasPrice': gasPrice });
                const gasLimit = 'gasLimit' in chain ? chain.gasLimit : 99999999999;
                this.depositAmountForm.patchValue({ 'gasLimit': gasLimit });
                if (this.coin.tokenType === 'BNB' || this.coin.tokenType === 'MATIC') {
                    const gasLimitToken = 'gasLimitToken' in chain ? chain.gasLimitToken : 99999999999;
                    this.depositAmountForm.patchValue({ 'gasLimit': gasLimitToken });
                }
    ...
    show() {
        const coinName = this.coin.name as keyof typeof environment.depositMinimumConfirmations;
        this.confirmations = environment.depositMinimumConfirmations[coinName];
