...
export class SpringComponent implements OnInit {
    ...
    for (let i = 0; i < this.bidFAB.length; i++) {
      ...
      if (i === 0) {
        if (item.quantity >= 30000) {
          item.reward = 850;
        } else if (item.quantity >= 20000) {
          item.reward = 550;
        } else if (item.quantity >= 10000) {
          item.reward = 350;
        } else if (item.quantity >= 5000) {
          item.reward = 150;
        } else if (item.quantity >= 2000) {
          item.reward = 80;
        }
      } else
        if (i === 1) {
    ...
    for (let i = 0; i < this.bidEXG.length; i++) {
      ...
      if (i === 0) {
        if (item.quantity >= 80000) {
          item.reward = 850;
        } else if (item.quantity >= 50000) {
          item.reward = 550;
        } else if (item.quantity >= 30000) {
          item.reward = 350;
        } else if (item.quantity >= 10000) {
          item.reward = 150;
        } else if (item.quantity >= 5000) {
  ...
  getTradesTotal(address: string) {
    const trades = this.trades.filter((item: any) => item.address1 == address || item.address2 == address);
    let total = 0;
  ...
  async springFestival() {
    ...
    forkJoin([this.httpServ.getRaw(url1), this.httpServ.getRaw(url2), this.httpServ.getRaw(url3)])
      .subscribe((results: any) => {
        ...
        this.bidFAB.sort(function (a: { quantity: number }, b: { quantity: number }): number {
          return b.quantity - a.quantity;
        });

        this.bidEXG.sort(function (a: { quantity: number }, b: { quantity: number }): number {
          return b.quantity - a.quantity;
        });

        this.depositUSDT.sort(function (a: { quantity: number }, b: { quantity: number }): number {
          return b.quantity - a.quantity;
        });

        this.bidFAB = this.bidFAB.filter((item: any) => item.quantity > 0);
        this.bidEXG = this.bidEXG.filter((item: any) => item.quantity > 0);
        this.depositUSDT = this.depositUSDT.filter((item: any) => item.quantity > 0);
