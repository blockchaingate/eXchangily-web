...
export class MetaforceComponent implements OnInit {
    ...
    this.apiServ.callFabSmartContract(address, abiHex, owner)
      .subscribe((res: any) => {
        ...
        if (res && res.executionResult) {
          ...
          if (output) {
            const hashes = this.web3Serv.decodeParameters(['bytes32[]'], output)[0] as any[];
            if (hashes && (hashes as any[]).length > 0) {
  ...
  withdrawable(blocknumber2: number) {
